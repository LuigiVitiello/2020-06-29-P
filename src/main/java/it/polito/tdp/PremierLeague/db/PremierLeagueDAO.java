...
public class PremierLeagueDAO {
	...
	public List<Adiacenza> getAdiacenze(Map<Integer,Match> idMap, int min, int mese){
		String sql="SELECT m1.MatchID AS m1, m2.MatchID AS m2 , ( COUNT( DISTINCT(a1.PlayerID)) + COUNT( DISTINCT(a2.PlayerID)) ) AS peso "
				+ "FROM actions a1, actions a2, matches m1, matches m2 "
				+ "WHERE a1.MatchID > a2.MatchID "
				+ "	AND m1.MatchID=a1.MatchID AND m2.MatchID=a2.MatchID AND a1.PlayerID=a2.PlayerID "
		...
		try {
			...
			while (res.next()) {
				if (idMap.containsKey(res.getInt("m1")) && idMap.containsKey(res.getInt("m2"))) {
					Adiacenza a = new Adiacenza(idMap.get(res.getInt("m1")),idMap.get(res.getInt("m2")),res.getInt("peso"));
					result.add(a);
				}
			}
			conn.close();
